...
contract AgreementManagerETH is AgreementManager {
    ...
    function depositB(uint agreementID) external payable {
        ...
        // We don't need to check that party A paid their stake, because they can't create an 
    ...
    function resolveAsParty(uint agreementID, uint resolutionWei) external {
        AgreementDataETH storage agreement = agreements[agreementID];
    ...
    function withdraw(uint agreementID) external {
        ...
        require(!partyWithdrew(agreement, callingParty), "Calling party already withdrew.");
        ...
        uint withdrawWei = add(
            toWei(withdrawAmount), 
            getPartyArbitrationRefundInWei(agreementID, agreement, callingParty)
        );
    ...
    function requestDefaultJudgment(uint agreementID) external {
        ...
        agreement.resolution = partyResolution(agreement, callingParty);
    ...
    function agreementIsOpen(AgreementDataETH storage agreement) internal view returns (bool) {
        return agreement.resolution == RESOLUTION_NULL && !partyWithdrew(agreement, Party.A);
